    ...
    <div className='my-10'>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 gap-y-6'>
        {wishlistItems.map((item, index) => (
          <ProductItem key={index} id={item._id} image={item.image} name={item.name} price={item.price} sizes={item.sizes} discount={item.discount} discountedPrice={item.discountedPrice} />
        ))}
      </div>
